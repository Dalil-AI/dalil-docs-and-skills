---
description: Interview a customer and generate their personalized Dalil Sales OS
...
customer in *this* conversation, then hand off to the `sales-os-generator` subagent to
produce their Sales OS — a single, self-contained HTML reference document
...
The Sales OS has 6 sections you must collect information for:
...
## Step 1 — Detect an existing Sales OS

- If `$ARGUMENTS` is provided, treat it as the path to an existing Sales OS file to refine.
- Otherwise, glob for `*.html` at the project root. A Sales OS file contains markers such
  as `Sales OS`, `s-strategy`, or `s-sequences`. **Skip** `template.html`, `index.html`,
  and `icp-matrix.html` — those are references, not the customer's file.

If an existing Sales OS is found, greet the customer, mention the file by name, and ask:
...
2. Invoke the **`sales-os-generator`** subagent via the Task tool — it ships with this
   plugin. (If your Claude Code namespaces plugin subagents, it may be listed as
   `dalil:sales-os-generator`; use whichever name the Task tool resolves.) The prompt
